...
class CustomerController extends Controller {
        ...
        if ($request->ajax()) {
            return DataTables::of($customers)
                ...
                ->addColumn('gender', function ($data) {
                    return $data->sex == null ? 'N/A' : $data->sex;
                })
                ->addColumn('contact', function ($data) {
                    return $data->contact == null ? 'N/A' : $data->contact;
                })
                ->addColumn('address', function ($data) {
                    return $data->address == null ? 'N/A' : $data->address;
                })
                ->addColumn('action', function ($data) {
                    $actionBtn = "<button type='button' class='btn btn-outline-dark' onclick='viewCustomer($data->customer_id)'>
                                    <i class='fa fa-eye'></i>
                                </button>
                                <button onclick='getOrder($data->customer_id)'  class='btn btn-outline-info'>
    ...
    public function customerOrderByRestaurant(Request $request) {
        ...
        $orders     = Order::where('customer_id', $request->customer_id)->where('restaurant_id', $request->id)->with('customer','status','items','order_combos')->latest()->get();
    ...
    public function ordersData($orders) {
        foreach ($orders as $order) {
            ...
            $order->customer_name    = $order->is_default_name == 1 ? $order->name : $order->customer->name;
            $order->customer_contact = (($order->is_default_contact == 1) ? $order->contact : (($order->customer->contact == null) ? 'N/A' :$order->customer->contact));
            $order->customer_address = $order->is_default_address == 1 ? $order->address : $order->customer->address;
        }
