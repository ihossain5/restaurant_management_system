    ...
    <select name="" class="form-control restaurant">
        @foreach ($restaurants as $restaurant)
            <option value="{{ $restaurant->restaurant_id }}"
                {{ $new_restaurant->restaurant_id == $restaurant->restaurant_id ? 'selected' : '' }}>{{ $restaurant->name }}
            </option>
        @endforeach
    </select>
@endsection
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success === true) {
                        ...
                        item_url = item_url.replace(':id', response.data.session_id);
                         $('.item_link').attr("href", item_url);

                        var category_url = '{{ route('item.category', ':id') }}';
                        category_url = category_url.replace(':id', response.data.session_id);
                         $('.category_link').attr("href", category_url);

                        var order_url = '{{ route('orders.today', ':id') }}';
                        order_url = order_url.replace(':id', response.data.session_id);
                         $('.order_link').attr("href", order_url);

                        if($.trim(response.data) ){
                            var orderTable = $('#orderTable').DataTable();
                            $.each(response.data.all_orders,function(key,val){
                             $.each(val,function(i,data){
                                 console.log(data);
                                var trDOM = orderTable.row.add([
                                        "" + data.name + "",
                                        "" + data.status.name + "",
                                        "" + data.is_default_name == 0 ? data.name : data.customer.name + "",
                                        "" + data.is_default_contact == 0 ? data.contact : data.customer.contact + "",
                                        "" + data.is_default_address == 0 ? data.address : data.customer.address + "",
                                        "" + data.amount + "",
                                        `   <button type='button' class='btn btn-outline-dark' onclick='viewOrder(${data.order_id})'>
                                                <i class='fa fa-eye'></i>
                                            </button>`
                                    ]).draw().node();
                                    $(trDOM).addClass('order' + data.order_id + '');
                              });
                          });
                        }
